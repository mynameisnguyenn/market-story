"""Tests for the shared pure figure/table builders in src/dashboard/charts.py."""
...
def test_section_records_yield_converts_to_bps():
    frame = charts.section_records(_yield_rows(), "yield")
    assert frame.loc[0, "1D"] == -2.0          # -0.02 pp -> -2 bps


def test_section_records_equity_uses_change_pct():
    frame = charts.section_records(_equity_rows(), "equity")
...
def test_section_styler_renders_with_missing_values():
    html = charts.section_styler(_equity_rows(), "equity").to_html()
    assert "Tech" in html and "n/a" in html     # NaN formatted as n/a, no crash


def test_section_styler_handles_empty_rows():
    html = charts.section_styler([], "equity").to_html()  # empty group must not crash
...
def test_macro_styler_handles_none_change():
    ...
    html = charts.macro_styler(macro).to_html()
    assert "10Y" in html and "CPI" in html


def test_sector_treemap_fig_shows_change_value():
    fig = charts.sector_treemap_fig(_equity_rows())
    ...
    assert charts.sector_treemap_fig([]) is None
...
def test_yield_curve_fig_builds_and_needs_two_points():
    ...
    fig = charts.yield_curve_fig(rates)
    assert fig is not None and len(fig.data) == 1
    assert list(fig.data[0].x) == [0.25, 10.0, 30.0]          # sorted by maturity
    assert charts.yield_curve_fig([{"symbol": "^TNX", "last": 4.45}]) is None  # one point
    assert charts.yield_curve_fig([]) is None
...
def test_compute_correlations_perfect_and_shape():
    ...
    corr = charts.compute_correlations(closes, ["A", "B"], window=60)
    assert corr is not None and corr.shape == (2, 2)
    assert corr.loc["A", "A"] == pytest.approx(1.0)
    assert corr.loc["A", "B"] == pytest.approx(1.0)
    assert charts.compute_correlations({"A": a}, ["A"], 60) is None   # need >= 2 series
...
def test_correlation_fig_builds_and_handles_thin():
    ...
    assert charts.correlation_fig({"A": a, "B": a * 2.0}, insts) is not None
    assert charts.correlation_fig({}, insts) is None                 # no data -> None
...
def test_sparkline_fig_builds_and_handles_short():
    ...
    fig = charts.sparkline_fig(s)
    assert fig is not None and len(fig.data) == 1
    assert charts.sparkline_fig(pd.Series([1.0])) is None     # too short
    assert charts.sparkline_fig(None) is None
...
def test_line_fig_builds_and_handles_empty():
    ...
    assert charts.line_fig(series, "S&P 500") is not None
    assert charts.line_fig(None, "S&P 500") is None
    assert charts.line_fig(pd.Series([], dtype=float), "S&P 500") is None
